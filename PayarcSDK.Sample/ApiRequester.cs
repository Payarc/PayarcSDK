...
public class ApiRequester
{
	...
	public async Task ListInstructionalFundings()
	{
		try
		{
			var responseData = await _payarc.InstructionalFunding.List(new BaseListOptions { });
			Console.WriteLine(JsonConvert.SerializeObject(responseData, new JsonSerializerSettings
			{
				NullValueHandling = NullValueHandling.Ignore,
				Formatting = Formatting.Indented
	...
	public async Task CreateInstructionalFunding()
	{
		try
		{
			var options = new Entities.InstructionalFunding.InstructionalFundingRequestData
			{
				Mid = "0709900000098856",
				Amount = 60
			};
			var responseData = await _payarc.InstructionalFunding.Create(options);
			Console.WriteLine("Instructional Funding Data");
